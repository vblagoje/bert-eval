...
class BertGeneration(object):
    ...
    # 2. repeatedly pick a location, mask the token at that location, and generate from the probability distribution
    # given by BERT
    ...
    # The `generate` function wraps and batches these three generation modes. In practice, we find that the first
    # leads to the most fluent samples.
...
def main(args):
    ...
    # - temperature (1.0): smoothing parameter for the next word distribution. Higher means more like uniform;
    # lower means more peaky
    # - burnin (250): for non-sequential generation, for the first burnin steps, sample from the entire next
    # word distribution, instead of top_k
    # - max_iter (500): number of iterations to run for
    # - seed_text (["CLS"]): prefix to generate for. We found it crucial to start with the CLS token; you can
    # try adding to it
